...
def list_users_equifilter(attributename, filterval):
    ...
    try:
        ...
        sql = f"""SELECT *
                    FROM users
                    WHERE  %s = %s """
        val = dictfetchall(cur,sql,(attributename,filterval,))
    except:
        # If there are any errors, we print something nice and return a null value
        import traceback
        traceback.print_exc()
...
def delete_airport(airportid):
    """
    Remove a airport from your system
    """
    # Data validation checks are assumed to have been done in route processing
    conn = database_connect()
    if(conn is None):
...
def add_airport(airportid, name, iatacode,city,country):
    ...
    try:
        ...
        # If there were any errors, return a NULL row printing an error to the debug
        print("Unexpected error adding a airport:", sys.exc_info()[0])
        cur.close()                     # Close the cursor
        conn.close()                    # Close the connection to the db
        raise

def search_airports_customfilter(filtertype, filterval):
    # Get the database connection and set up the cursor
    conn = database_connect()
    if(conn is None):
        # If a connection cannot be established, send an Null object
    ...
    try:
        ...
        sql = f"""SELECT *
                    FROM airports
                    WHERE airportid {filtertype} {filtervalprefix}%s{filtervalsuffix} """
        print_sql_string(sql, (filterval,))
        val = dictfetchall(cur,sql,(filterval,))
    except:
        # If there are any errors, we print something nice and return a null value
...
# Get all rows in aiports where a particular attribute matches a value
...
def update_single_airport(airportid, name, iatacode,city,country):
    ...
    try:
        ...
        sql = f"""UPDATE airports
                    SET {setitems}
                    WHERE airportid = %s;"""
        print_sql_string(sql,(name, iatacode,city,country, airportid))
        val = dictfetchone(cur,sql,(name, iatacode,city,country,airportid))
        conn.commit()
...
def airport_summary():
    ...
    try:
        # Set-up our SQL query
        sql = """SELECT country, COUNT(airportid) as count
                    FROM airports 
                    GROUP BY country
                    ORDER BY COUNT(airportid) DESC, country ASC """
...
def airport_summary_city():
    ...
    try:
        # Set-up our SQL query
        sql = """SELECT city, COUNT(airportid) as count
                    FROM airports 
                    GROUP BY city
                    ORDER BY COUNT(airportid) DESC, city ASC """
